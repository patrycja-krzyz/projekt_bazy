...
def random_work_datetime(dates):
...
def generate_payment_ticket(baza) -> pd.DataFrame:
    payments = []
    tickets = []
    df_prices = pd.read_sql("SELECT * FROM prices", con=baza.con)
    all_tickets = df_prices["ticket_id"].tolist()
    for i in range(1,1001):
        k = random.choices(range(1, 6), weights=[0.2, 0.3, 0.2, 0.2, 0.1], k=1)[0]
...
def generate_payments(baza, payments_tickets) -> pd.DataFrame:
    payment_ids = payments_tickets["payment_id"].dropna().unique().tolist()
    dates = []
    amounts = []
    guests_df = pd.read_sql("SELECT * FROM guests", con=baza.con)
    guests = guests_df.loc[guests_df["birth_date"] <= pd.Timestamp('2010-01-01').date(), "guest_id"].to_list()
